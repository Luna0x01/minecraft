...
public class RealmsWorldSlotButton extends RealmsButton {
	...
	public void tick() {
		...
		if (realmsServer != null) {
			...
			String string5 = null;
			RealmsWorldSlotButton.Action action2;
			if (bl2) {
				...
				if (!realmsServer.expired && bl6) {
					action2 = RealmsWorldSlotButton.Action.JOIN;
					string5 = Realms.getLocalizedString("mco.configure.world.slot.tooltip.active");
				} else {
					action2 = RealmsWorldSlotButton.Action.NOTHING;
				}
			} else if (bl) {
				if (realmsServer.expired) {
					action2 = RealmsWorldSlotButton.Action.NOTHING;
				} else {
					action2 = RealmsWorldSlotButton.Action.SWITCH_SLOT;
			...
			this.state = new RealmsWorldSlotButton.State(bl2, string, l, string2, bl3, bl, action2, string5);
			String string6;
			if (action2 == RealmsWorldSlotButton.Action.NOTHING) {
				string6 = string;
			} else if (bl) {
				if (bl3) {
					string6 = string5;
				} else {
					string6 = string5 + " " + string + " " + realmsServer.minigameName;
				}
			} else {
