...
public class RecipeUnlockedCriterion implements Criterion<RecipeUnlockedCriterion.Conditions> {
	...
	private final Map<PlayerAdvancementTracker, RecipeUnlockedCriterion.Handler> handlers = Maps.newHashMap();
	...
	static class Handler {
		...
		private final Set<Criterion.ConditionsContainer<RecipeUnlockedCriterion.Conditions>> conditions = Sets.newHashSet();
		...
		public void handle(Recipe<?> recipe) {
			...
			if (list != null) {
				for (Criterion.ConditionsContainer<RecipeUnlockedCriterion.Conditions> conditionsContainer2 : list) {
