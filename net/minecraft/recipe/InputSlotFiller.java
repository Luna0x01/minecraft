...
public class InputSlotFiller<C extends Inventory> implements RecipeGridAligner<Integer> {
	...
	protected void fillInputSlots(Recipe<C> recipe, boolean craftAll) {
		...
		int k = this.getAmountToFill(craftAll, i, bl);
		...
		if (this.matcher.match(recipe, intList, k)) {
			...
			while (var8.hasNext()) {
				...
				int n = RecipeMatcher.getStackFromId(m).getMaxCount();
				if (n < l) {
					l = n;
				}
			}

			if (this.matcher.match(recipe, intList, l)) {
				this.returnInputs(false);
				this.alignRecipeToGrid(
					this.handler.getCraftingWidth(), this.handler.getCraftingHeight(), this.handler.getCraftingResultSlotIndex(), recipe, intList.iterator(), l
				);
			}
		}
	}
