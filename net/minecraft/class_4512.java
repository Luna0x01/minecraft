...
public class class_4512 extends DataFix {
	private static final Map<String, Integer> field_22328 = (Map<String, Integer>)DataFixUtils.make(Maps.newHashMap(), hashMap -> {
	...
	public class_4512(Schema schema, boolean bl) {
		super(schema, bl);
	}

	public static int method_21694(String string) {
		Integer integer = (Integer)field_22328.get(string);
	...
	public TypeRewriteRule makeRule() {
		...
		Function<Typed<?>, Typed<?>> function = typed -> this.method_21689(typed, "DisplayTile", "DisplayData", "DisplayState");
		Function<Typed<?>, Typed<?>> function2 = typed -> this.method_21689(typed, "inTile", "inData", "inBlockState");
		Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> type = DSL.and(
			DSL.optional(DSL.field("inTile", DSL.named(class_3402.field_16597.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())))), DSL.remainderType()
		);
		Function<Typed<?>, Typed<?>> function3 = typed -> typed.update(type.finder(), DSL.remainderType(), Pair::getSecond);
		return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(class_3402.field_16596), schema2.getType(class_3402.field_16596), typed -> {
			typed = this.method_21690(typed, "minecraft:falling_block", this::method_21688);
			typed = this.method_21690(typed, "minecraft:enderman", typedx -> this.method_21689(typedx, "carried", "carriedData", "carriedBlockState"));
			typed = this.method_21690(typed, "minecraft:arrow", function2);
			typed = this.method_21690(typed, "minecraft:spectral_arrow", function2);
			typed = this.method_21690(typed, "minecraft:egg", function3);
			typed = this.method_21690(typed, "minecraft:ender_pearl", function3);
			typed = this.method_21690(typed, "minecraft:fireball", function3);
			typed = this.method_21690(typed, "minecraft:potion", function3);
			typed = this.method_21690(typed, "minecraft:small_fireball", function3);
			typed = this.method_21690(typed, "minecraft:snowball", function3);
			typed = this.method_21690(typed, "minecraft:wither_skull", function3);
			typed = this.method_21690(typed, "minecraft:xp_bottle", function3);
			typed = this.method_21690(typed, "minecraft:commandblock_minecart", function);
			typed = this.method_21690(typed, "minecraft:minecart", function);
			typed = this.method_21690(typed, "minecraft:chest_minecart", function);
			typed = this.method_21690(typed, "minecraft:furnace_minecart", function);
			typed = this.method_21690(typed, "minecraft:tnt_minecart", function);
			typed = this.method_21690(typed, "minecraft:hopper_minecart", function);
			return this.method_21690(typed, "minecraft:spawner_minecart", function);
		});
	}

	private Typed<?> method_21688(Typed<?> typed) {
		Type<Either<Pair<String, Either<Integer, String>>, Unit>> type = DSL.optional(
			DSL.field("Block", DSL.named(class_3402.field_16597.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())))
		);
		Type<Either<Pair<String, Dynamic<?>>, Unit>> type2 = DSL.optional(DSL.field("BlockState", DSL.named(class_3402.field_16593.typeName(), DSL.remainderType())));
		Dynamic<?> dynamic = (Dynamic<?>)typed.get(DSL.remainderFinder());
		return typed.update(type.finder(), type2, either -> {
			int i = (Integer)either.map(pair -> (Integer)((Either)pair.getSecond()).map(integer -> integer, class_4512::method_21694), unit -> {
			...
			return Either.left(Pair.of(class_3402.field_16593.typeName(), class_4500.method_21605(i << 4 | j)));
		}).set(DSL.remainderFinder(), dynamic.remove("Data").remove("TileID").remove("Tile"));
	}

	private Typed<?> method_21689(Typed<?> typed, String string, String string2, String string3) {
		Type<Pair<String, Either<Integer, String>>> type = DSL.field(
			string, DSL.named(class_3402.field_16597.typeName(), DSL.or(DSL.intType(), DSL.namespacedString()))
		);
		Type<Pair<String, Dynamic<?>>> type2 = DSL.field(string3, DSL.named(class_3402.field_16593.typeName(), DSL.remainderType()));
		Dynamic<?> dynamic = (Dynamic<?>)typed.getOrCreate(DSL.remainderFinder());
		return typed.update(type.finder(), type2, pair -> {
			int i = (Integer)((Either)pair.getSecond()).map(integer -> integer, class_4512::method_21694);
			int j = dynamic.getInt(string2) & 15;
			return Pair.of(class_3402.field_16593.typeName(), class_4500.method_21605(i << 4 | j));
		}).set(DSL.remainderFinder(), dynamic.remove(string2));
	}

	private Typed<?> method_21690(Typed<?> typed, String string, Function<Typed<?>, Typed<?>> function) {
		Type<?> type = this.getInputSchema().getChoiceType(class_3402.field_16596, string);
		Type<?> type2 = this.getOutputSchema().getChoiceType(class_3402.field_16596, string);
