...
public class TestUtil {
	...
	public static void startTest(GameTest gameTest, TestManager testManager) {
		...
		gameTest.addListener(new TestListener() {
			...
			public void onStarted(GameTest gameTest) {
				TestUtil.createBeacon(gameTest, Blocks.field_9996);
			}

			@Override
			public void onFailed(GameTest gameTest) {
				TestUtil.createBeacon(gameTest, gameTest.isRequired() ? Blocks.field_10272 : Blocks.field_10227);
	...
	private static void handleTestFail(GameTest gameTest) {
		...
		sendMessage(gameTest.getWorld(), Formatting.field_1061, string);
	...
	private static void createBeacon(GameTest gameTest, Block block) {
		...
		serverWorld.setBlockState(blockPos2, Blocks.field_10327.getDefaultState());
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				...
				serverWorld.setBlockState(blockPos4, Blocks.field_10085.getDefaultState());
			}
		}
	}
	...
	private static void createLectern(GameTest gameTest, String string) {
		...
		BlockPos blockPos = gameTest.getPos();
		BlockPos blockPos2 = blockPos.add(-1, 1, -1);
		serverWorld.setBlockState(blockPos2, Blocks.field_16330.getDefaultState());
		BlockState blockState = serverWorld.getBlockState(blockPos2);
		ItemStack itemStack = createBook(gameTest.getStructurePath(), gameTest.isRequired(), string);
	...
	public static void clearTests(ServerWorld serverWorld, BlockPos blockPos, TestManager testManager, int i) {
		...
		BlockPos.stream(blockPos2, blockPos3).filter(blockPosx -> serverWorld.getBlockState(blockPosx).getBlock() == Blocks.field_10465).forEach(blockPosx -> {
