	...
	protected TypeRewriteRule makeRule() {
		return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), typed -> {
			...
			for (String string2 : TAMEABLE_PETS) {
				typed = this.updateTyped(typed, string2, EntityUuidFix::updateTameable);
			}

			for (String string3 : BREEDABLES) {
				typed = this.updateTyped(typed, string3, EntityUuidFix::updateBreedable);
			}

			for (String string4 : LEASHABLES) {
				typed = this.updateTyped(typed, string4, EntityUuidFix::updateLeashable);
			}

			for (String string5 : OTHER_LIVINGS) {
				typed = this.updateTyped(typed, string5, EntityUuidFix::updateLiving);
			}

			for (String string6 : PROJECTILES) {
				typed = this.updateTyped(typed, string6, EntityUuidFix::updateProjectile);
			}
