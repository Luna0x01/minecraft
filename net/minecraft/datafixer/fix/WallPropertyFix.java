...
public class WallPropertyFix extends DataFix {
	...
	private static <T> Dynamic<T> updateWallValueReference(Dynamic<T> dynamic, String string) {
		return dynamic.update(
			string, dynamicx -> (Dynamic)DataFixUtils.orElse(dynamicx.asString().result().map(WallPropertyFix::booleanToWallType).map(dynamicx::createString), dynamicx)
		);
	}
	...
	private static <T> Dynamic<T> updateWallProperties(Dynamic<T> dynamic) {
		...
		return !bl ? dynamic : dynamic.update("Properties", dynamicx -> {
			...
			dynamic2 = updateWallValueReference((Dynamic<T>)dynamic2, "west");
			dynamic2 = updateWallValueReference((Dynamic<T>)dynamic2, "north");
			return updateWallValueReference((Dynamic<T>)dynamic2, "south");
		});
	}
}
