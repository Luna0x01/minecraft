...
public class class_3386 extends DataFix {
	private static final Int2ObjectMap<String> field_16554 = (Int2ObjectMap<String>)DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
	...
	public class_3386(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	protected TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
			"ItemStackEnchantmentFix", type, typed -> typed.updateTyped(opticFinder, typedx -> typedx.update(DSL.remainderFinder(), this::method_15124))
		);
	}

	private Dynamic<?> method_15124(Dynamic<?> dynamic) {
		Optional<Dynamic<?>> optional = dynamic.get("ench")
			.flatMap(Dynamic::getStream)
			.map(stream -> stream.map(dynamicx -> dynamicx.set("id", dynamicx.createString((String)field_16554.getOrDefault(dynamicx.getInt("id"), "null")))))
		...
		return dynamic.update(
			...
			dynamicx -> (Dynamic)DataFixUtils.orElse(
					dynamicx.getStream()
						.map(stream -> stream.map(dynamicxx -> dynamicxx.set("id", dynamicxx.createString((String)field_16554.getOrDefault(dynamicxx.getInt("id"), "null")))))
