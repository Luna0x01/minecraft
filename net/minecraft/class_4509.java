...
public class class_4509 extends DataFix {
	private static final ImmutableMap<String, Pair<String, ImmutableMap<String, String>>> field_22327 = ImmutableMap.builder()
		.put(
			"EndCity",
			Pair.of(
				"ECP", ImmutableMap.builder().put("second_floor", "second_floor_1").put("third_floor", "third_floor_1").put("third_floor_c", "third_floor_2").build()
			)
		)
		.put(
			"Mansion",
	...
	public class_4509(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
			...
			typed -> typed.updateTyped(
					opticFinder, typed2 -> typed2.update(DSL.remainderFinder(), dynamic -> this.method_21672((Dynamic<?>)typed.get(DSL.remainderFinder()), dynamic))
				)
		);
	}

	private Dynamic<?> method_21672(Dynamic<?> dynamic, Dynamic<?> dynamic2) {
		String string = dynamic.getString("id");
		if (field_22327.containsKey(string)) {
			Pair<String, ImmutableMap<String, String>> pair = (Pair<String, ImmutableMap<String, String>>)field_22327.get(string);
			if (((String)pair.getFirst()).equals(dynamic2.getString("id"))) {
				String string2 = dynamic2.getString("Template");
