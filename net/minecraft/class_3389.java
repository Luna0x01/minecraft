...
public class class_3389 extends DataFix {
	private static final Map<String, String> field_16556 = (Map<String, String>)DataFixUtils.make(Maps.newHashMap(), hashMap -> {
	...
	private static final Set<String> field_16557 = (Set<String>)field_16556.keySet()
		.stream()
		.map(string -> string.substring(0, string.indexOf(46)))
		.collect(Collectors.toSet());
	private static final Set<String> field_16558 = Sets.newHashSet(
		new String[]{
			"minecraft:bow",
	...
	public class_3389(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", type, typed -> {
			...
			if (!optional.isPresent()) {
				...
				String string = method_15138((String)((Pair)optional.get()).getSecond(), i);
				if (string != null) {
					typed2 = typed.set(opticFinder, Pair.of(class_3402.field_16598.typeName(), string));
				}

				if (field_16558.contains(((Pair)optional.get()).getSecond())) {
	...
	@Nullable
	public static String method_15138(@Nullable String string, int i) {
		if (field_16557.contains(string)) {
			String string2 = (String)field_16556.get(string + '.' + i);
			return string2 == null ? (String)field_16556.get(string + ".0") : string2;
		} else {
			return null;
		}
	}
