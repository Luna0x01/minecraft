...
public abstract class VoxelShape {
	...
	protected double getCoord(Direction.Axis getMax, int i) {
		return this.getIncludedPoints(getMax).getDouble(i);
	}
	...
	public double getBoundingBoxes(Direction.Axis axis, double d, double e) {
		...
		int k = this.voxels.method_18029(axis, i, j);
		return k >= this.voxels.getSize(axis) ? Double.POSITIVE_INFINITY : this.getCoord(axis, k);
	}

	public double method_18084(Direction.Axis axis, double d, double e) {
		Direction.Axis axis2 = AxisCycleDirection.FORWARD.cycle(axis);
		Direction.Axis axis3 = AxisCycleDirection.BACKWARD.cycle(axis);
		int i = this.getCoordIndex(axis2, d);
		int j = this.getCoordIndex(axis3, e);
		int k = this.voxels.getSize(axis, i, j);
		return k <= 0 ? Double.NEGATIVE_INFINITY : this.getCoord(axis, k);
	}

	protected int getCoordIndex(Direction.Axis axis, double d) {
		return MathHelper.method_21513(0, this.voxels.getSize(axis) + 1, i -> {
			if (i < 0) {
				return false;
			} else {
	...
	@Nullable
	public BlockHitResult rayTrace(Vec3d vec3d, Vec3d vec3d2, BlockPos pos) {
		if (this.isEmpty()) {
			return null;
		} else {
			Vec3d vec3d3 = vec3d2.subtract(vec3d);
			if (vec3d3.squaredLength() < 1.0E-7) {
				return null;
			} else {
				Vec3d vec3d5 = vec3d.add(vec3d3.multiply(0.001));
				Vec3d vec3d6 = vec3d.add(vec3d3.multiply(0.001)).subtract((double)pos.getX(), (double)pos.getY(), (double)pos.getZ());
				return this.contains(vec3d6.x, vec3d6.y, vec3d6.z)
					? new BlockHitResult(vec3d5, Direction.getFacing(vec3d3.x, vec3d3.y, vec3d3.z), pos)
					: Box.rayTrace(this.getBoundingBoxes(), vec3d, vec3d2, pos);
			}
		}
	}

	public VoxelShape getFace(Direction direction) {
		if (!this.isEmpty() && this != VoxelShapes.matchesAnywhere()) {
	...
	public double method_18075(Direction.Axis axis, Box box, double d) {
		return this.method_18069(AxisCycleDirection.between(axis, Direction.Axis.X), box, d);
	}
					...
					for (int t = k; t < l; t++) {
						for (int u = m; u < n; u++) {
							if (this.voxels.inBoundsAndContains(axisCycleDirection2, s, t, u)) {
								double h = this.getCoord(axis, s + 1) - f;
								if (h <= 1.0E-7) {
									d = Math.max(d, h);
								}
