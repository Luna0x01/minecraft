...
public class SubtitlesHud extends DrawableHelper implements ListenerSoundInstance {
	...
	public void render() {
		...
		if (this.enabled && !this.entries.isEmpty()) {
			...
			for (SubtitlesHud.SubtitleEntry subtitleEntry2 : this.entries) {
				int k = 255;
				...
				int p = MathHelper.floor(MathHelper.clampedLerp(255.0, 75.0, (double)((float)(SystemUtil.getMeasuringTimeMs() - subtitleEntry2.getTime()) / 3000.0F)));
				int q = p << 16 | p << 8 | p;
				GlStateManager.pushMatrix();
