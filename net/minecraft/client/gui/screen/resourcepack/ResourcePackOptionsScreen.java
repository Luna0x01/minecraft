...
public class ResourcePackOptionsScreen extends Screen {
	...
	protected void init() {
		...
		this.addButton(new ButtonWidget(this.width / 2 + 4, this.height - 48, 150, 20, I18n.translate("gui.done"), buttonWidget -> {
			if (this.edited) {
				List<ClientResourcePackContainer> listx = Lists.newArrayList();
				...
				for (ClientResourcePackContainer clientResourcePackContainer : listx) {
					if (!clientResourcePackContainer.isPositionFixed()) {
		...
		if (!this.edited) {
			...
			List<ClientResourcePackContainer> list = Lists.newArrayList(resourcePackContainerManager.getAlphabeticallyOrderedContainers());
			...
			for (ClientResourcePackContainer clientResourcePackContainer2 : Lists.reverse(Lists.newArrayList(resourcePackContainerManager.getEnabledContainers()))) {
				this.selectedList.addEntry(new ResourcePackListWidget.ResourcePackEntry(this.selectedList, this, clientResourcePackContainer2));
			}
		}
	}
