...
public abstract class ResourcePackWidget implements EntryListWidget.Entry {
	...
	public void render(int index, int x, int y, int rowWidth, int rowHeight, int mouseX, int mouseY, boolean hovered) {
		...
		if ((this.client.options.touchscreen || hovered) && this.isVisible()) {
		...
		if (this.isVisible() && x <= 32) {
			if (this.isNotSelected()) {
				...
				if (i != 1) {
					...
					String string2 = I18n.translate("resourcePack.incompatible.confirm." + (i > 1 ? "new" : "old"));
					this.client.setScreen(new ConfirmScreen(new IdentifiableBooleanConsumer() {
						...
						public void confirmResult(boolean confirmed, int id) {
							...
							if (confirmed) {
								...
								ResourcePackWidget.this.screen.getSelectedPacks().add(0, ResourcePackWidget.this);
							}
						}
					}, string, string2, 0));
				} else {
					this.screen.getContainingList(this).remove(this);
					this.screen.getSelectedPacks().add(0, this);
				}
