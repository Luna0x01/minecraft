...
public class MerchantScreen extends HandledScreen<MerchantScreenHandler> {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		if (!tradeOfferList.isEmpty()) {
			...
			TradeOffer tradeOffer2 = (TradeOffer)tradeOfferList.get(o);
			if (this.handler.isLeveled()) {
				this.drawLevelInfo(matrices, i, j, tradeOffer2);
			}

			if (tradeOffer2.isDisabled() && this.isPointWithinBounds(186, 35, 22, 21, (double)mouseX, (double)mouseY) && this.handler.canRefreshTrades()) {
		...
		public void renderTooltip(MatrixStack matrices, int mouseX, int mouseY) {
			if (this.hovered && MerchantScreen.this.handler.getRecipes().size() > this.index + MerchantScreen.this.indexStartOffset) {
				if (mouseX < this.x + 20) {
					...
					ItemStack itemStack2 = ((TradeOffer)MerchantScreen.this.handler.getRecipes().get(this.index + MerchantScreen.this.indexStartOffset)).getSecondBuyItem();
