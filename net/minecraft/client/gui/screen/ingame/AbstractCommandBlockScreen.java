	...
	class SuggestionWindow {
		...
		public void draw(int i, int j) {
			...
			if (bl3) {
				...
				if (bl2) {
					for (int o = 0; o < this.area.getWidth(); o++) {
						if (o % 2 == 0) {
							DrawableHelper.fill(
								this.area.getX() + o, this.area.getY() + this.area.getHeight(), this.area.getX() + o + 1, this.area.getY() + this.area.getHeight() + 1, -1
							);
						}
					}
				}
			...
			for (int p = 0; p < k; p++) {
				Suggestion suggestion = (Suggestion)this.suggestions.getList().get(p + this.inWindowIndex);
				DrawableHelper.fill(this.area.getX(), this.area.getY() + 12 * p, this.area.getX() + this.area.getWidth(), this.area.getY() + 12 * p + 12, Integer.MIN_VALUE);
				if (i > this.area.getX() && i < this.area.getX() + this.area.getWidth() && j > this.area.getY() + 12 * p && j < this.area.getY() + 12 * p + 12) {
					if (bl4) {
						this.select(p + this.inWindowIndex);
					}
				...
				AbstractCommandBlockScreen.this.font
					.drawWithShadow(
						suggestion.getText(), (float)(this.area.getX() + 1), (float)(this.area.getY() + 2 + 12 * p), p + this.inWindowIndex == this.selection ? -256 : -5592406
					);
			}
