...
public abstract class ContainerScreen<T extends Container> extends Screen implements ContainerProvider<T> {
	...
	public ContainerScreen(T container, PlayerInventory playerInventory, Text text) {
		super(text);
	...
	public void render(int i, int j, float f) {
		...
		for (int o = 0; o < this.container.slots.size(); o++) {
			Slot slot = (Slot)this.container.slots.get(o);
		...
		if (!itemStack.isEmpty()) {
			...
			this.drawItem(itemStack, i - k - 8, j - l - s, string);
		}
		...
		if (!this.touchDropReturningStack.isEmpty()) {
			...
			int t = this.touchDropOriginSlot.xPosition - this.touchDropX;
			int u = this.touchDropOriginSlot.yPosition - this.touchDropY;
	...
	private void drawSlot(Slot slot) {
		...
		if (itemStack.isEmpty() && slot.doDrawHoveringEffect()) {
			...
			if (pair != null) {
				Sprite sprite = (Sprite)this.minecraft.getSpriteAtlas((Identifier)pair.getFirst()).apply(pair.getSecond());
		...
		if (this.isDoubleClicking && slot != null && i == 0 && this.container.canInsertIntoSlot(ItemStack.EMPTY, slot)) {
			...
			if (this.touchDragSlotStart != null && this.minecraft.options.touchscreen) {
				...
				for (Slot slot3 : this.cursorDragSlots) {
					this.onMouseClick(slot3, slot3.id, Container.packClickData(1, this.heldButtonType), SlotActionType.field_7789);
				}
				...
				if (this.minecraft.options.keyPickItem.matchesMouse(i)) {
					...
					this.onMouseClick(slot, l, i, bl3 ? SlotActionType.field_7794 : SlotActionType.field_7790);
				}
			}
		}
