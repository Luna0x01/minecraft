...
public class LoomScreen extends AbstractContainerScreen<LoomContainer> {
	...
	private static final List<DyeColor> PATTERN_BUTTON_COLORS = Lists.newArrayList(
		new DyeColor[]{PATTERN_BUTTON_BACKGROUND_COLOR, PATTERN_BUTTON_FOREGROUND_COLOR}
	);
	...
	public void tick() {
		...
		if (this.lastCachedPatternButtonTextureId < BannerPattern.COUNT) {
			...
			this.patternButtonTextureIds[this.lastCachedPatternButtonTextureId] = TextureCache.BANNER
				.get(string + string2, Lists.newArrayList(new BannerPattern[]{BannerPattern.BASE, bannerPattern}), PATTERN_BUTTON_COLORS);
	...
	protected void drawBackground(float f, int i, int j) {
		...
		if (this.canApplyDyePattern) {
			...
			int w = l + 13;
			this.minecraft.getTextureManager().bindTexture(TEXTURE);
			this.blit(v, w, 0, this.containerHeight, 14, 14);
			int x = this.container.getSelectedPattern();
			if (this.patternButtonTextureIds[x] != null) {
				this.minecraft.getTextureManager().bindTexture(this.patternButtonTextureIds[x]);
				blit(v + 4, w + 2, 5, 10, 1.0F, 1.0F, 20, 40, 64, 64);
			}
		}
	}
