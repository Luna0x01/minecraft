...
public class ChatScreen extends Screen {
	...
	private void updateCommand() {
		...
		if (stringReader.canRead() && stringReader.peek() == '/') {
			...
			int j = getLastWhitespaceIndex(string);
			Collection<String> collection = this.minecraft.player.networkHandler.getCommandSource().getPlayerNames();
			this.suggestionsFuture = CommandSource.suggestMatching(collection, new SuggestionsBuilder(string, j));
		}
	}
	...
	class SuggestionWindow {
		...
		public void draw(int i, int j) {
			...
			for (int o = 0; o < k; o++) {
				...
				DrawableHelper.fill(this.area.getX(), this.area.getY() + 12 * o, this.area.getX() + this.area.getWidth(), this.area.getY() + 12 * o + 12, -805306368);
