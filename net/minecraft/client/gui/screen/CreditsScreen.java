...
public class CreditsScreen extends Screen {
	...
	protected void init() {
		if (this.credits == null) {
			...
			try {
				...
				InputStream inputStream2 = this.minecraft.getResourceManager().getResource(new Identifier("texts/credits.txt")).getInputStream();
	...
	public void render(int i, int j, float f) {
		...
		BufferBuilder bufferBuilder = tessellator.getBuffer();
		bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
		bufferBuilder.vertex(0.0, (double)q, (double)this.getBlitOffset()).texture(0.0F, 1.0F).color(1.0F, 1.0F, 1.0F, 1.0F).next();
		bufferBuilder.vertex((double)p, (double)q, (double)this.getBlitOffset()).texture(1.0F, 1.0F).color(1.0F, 1.0F, 1.0F, 1.0F).next();
		bufferBuilder.vertex((double)p, 0.0, (double)this.getBlitOffset()).texture(1.0F, 0.0F).color(1.0F, 1.0F, 1.0F, 1.0F).next();
