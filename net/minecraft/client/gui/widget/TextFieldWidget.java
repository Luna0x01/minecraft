...
public class TextFieldWidget extends ClickableWidget implements Drawable, Element {
	...
	public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if (this.isVisible()) {
			...
			int j = this.editable ? this.editableColor : this.uneditableColor;
			...
			if (bl2) {
				if (bl3) {
					DrawableHelper.fill(matrices, p, n - 1, p + 1, n + 1 + 9, -3092272);
				} else {
			...
			if (l != k) {
				...
				this.drawSelectionHighlight(p, n - 1, q - 1, n + 1 + 9);
			}
		}
	}
