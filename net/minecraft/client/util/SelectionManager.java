...
public class SelectionManager {
	...
	public boolean handleSpecialKey(int i) {
		...
		if (Screen.isSelectAll(i)) {
			...
			int k = this.fontRenderer.isRightToLeft() ? -1 : 1;
			if (Screen.hasControlDown()) {
				this.selectionStart = this.fontRenderer.findWordEdge(string, k, this.selectionStart, true);
			} else {
				this.selectionStart = Math.max(0, Math.min(string.length(), this.selectionStart + k));
			}
