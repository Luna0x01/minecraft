...
public final class Rotation3 {
	...
	private static Matrix4f setup(@Nullable Vector3f vector3f, @Nullable Quaternion quaternion, @Nullable Vector3f vector3f2, @Nullable Quaternion quaternion2) {
		...
		if (vector3f2 != null) {
			matrix4f.multiply(Matrix4f.scale(vector3f2.getX(), vector3f2.getY(), vector3f2.getZ()));
		}
		...
		if (vector3f != null) {
			...
			matrix4f.a23 = vector3f.getZ();
		}

		return matrix4f;
	}

	public static Pair<Matrix3f, Vector3f> getLinearTransformationAndTranslationFromAffine(Matrix4f matrix4f) {
