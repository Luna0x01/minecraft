...
public class ClientRecipeBook extends RecipeBook {
		...
		for (Recipe<?> recipe : this.manager.values()) {
			if (!recipe.isIgnoredInRecipeBook()) {
				...
				if (string.isEmpty()) {
					...
					recipeResultCollection = (RecipeResultCollection)table.get(recipeBookGroup, string);
	...
	public static List<RecipeBookGroup> getGroupsForContainer(CraftingContainer<?> craftingContainer) {
		if (craftingContainer instanceof CraftingTableContainer || craftingContainer instanceof PlayerContainer) {
			...
			return Lists.newArrayList(
				new RecipeBookGroup[]{RecipeBookGroup.field_1804, RecipeBookGroup.field_1808, RecipeBookGroup.field_1811, RecipeBookGroup.field_1812}
			);
		} else if (craftingContainer instanceof BlastFurnaceContainer) {
			return Lists.newArrayList(new RecipeBookGroup[]{RecipeBookGroup.field_17110, RecipeBookGroup.field_17111, RecipeBookGroup.field_17112});
		} else {
			return craftingContainer instanceof SmokerContainer
				? Lists.newArrayList(new RecipeBookGroup[]{RecipeBookGroup.field_17113, RecipeBookGroup.field_17114})
				: Lists.newArrayList();
		}
	}
