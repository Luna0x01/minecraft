...
public class ChunkBatcher {
	...
	public ChunkBatcher(boolean bl) {
		...
		int p = Math.min(k, this.bufferCount);
		if (p > 1) {
			for (int q = 0; q < p; q++) {
	...
	public String getDebugString() {
		return this.workerThreads.isEmpty()
			...
			: String.format("pC: %03d, pU: %02d, aB: %02d", this.pendingChunks.size(), this.pendingUploads.size(), this.availableBuffers.size());
	}
	...
	public void reset() {
		this.clear();
		List<BlockLayeredBufferBuilder> list = Lists.newArrayList();
