	...
	protected void setupTransforms(PandaEntity pandaEntity, MatrixStack matrixStack, float f, float g, float h) {
		...
		if (pandaEntity.playingTicks > 0) {
			...
			if (i < 8) {
				...
				matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-s));
				...
				matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-w));
			} else if (i < 32) {
				...
				matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-aa));
			}
		}
		...
		if (ab > 0.0F) {
			...
			matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(MathHelper.lerp(ab, pandaEntity.getPitch(), pandaEntity.getPitch() + 90.0F)));
			...
			if (pandaEntity.isScaredByThunderstorm()) {
				...
				matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(ac));
		...
		if (ad > 0.0F) {
			...
			matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(MathHelper.lerp(ad, pandaEntity.getPitch(), pandaEntity.getPitch() + 180.0F)));
		}
	}
