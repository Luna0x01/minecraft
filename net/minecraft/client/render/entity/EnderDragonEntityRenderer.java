...
public class EnderDragonEntityRenderer extends EntityRenderer<EnderDragonEntity> {
	...
	public void render(EnderDragonEntity enderDragonEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
		...
		this.model.render(matrixStack, vertexConsumer4, i, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
		if (enderDragonEntity.ticksSinceDeath > 0) {
		...
		if (enderDragonEntity.connectedCrystal != null) {
			...
			renderCrystalBeam(
				r, s + EndCrystalEntityRenderer.getYOffset(enderDragonEntity.connectedCrystal, g), t, g, enderDragonEntity.age, matrixStack, vertexConsumerProvider, i
			);
			matrixStack.pop();
		}
	...
	public static class DragonEntityModel extends EntityModel<EnderDragonEntity> {
		...
		public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
			...
			matrices.pop();
			float r = -((float)Math.sin((double)(f * (float) (Math.PI * 2)))) * 0.0F;
			n = f * (float) (Math.PI * 2);
			i = 10.0F;
			j = 60.0F;
			h = 0.0F;
			...
			for (int s = 0; s < 12; s++) {
				fs = this.dragon.getSegmentProperties(12 + s, this.tickDelta);
				r = (float)((double)r + Math.sin((double)((float)s * 0.45F + n)) * 0.05F);
				this.neck.yaw = (MathHelper.fwrapDegrees(fs[0] - ds[0]) * 1.5F + 180.0F) * (float) (Math.PI / 180.0);
				this.neck.pitch = r + (float)(fs[1] - ds[1]) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
