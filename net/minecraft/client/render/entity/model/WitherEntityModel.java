...
public class WitherEntityModel<T extends WitherEntity> extends CompositeEntityModel<T> {
	...
	public void setAngles(T witherEntity, float f, float g, float h, float i, float j) {
		...
		this.heads[0].yaw = i * (float) (Math.PI / 180.0);
		this.heads[0].pitch = j * (float) (Math.PI / 180.0);
	}

	public void animateModel(T witherEntity, float f, float g, float h) {
		for (int i = 1; i < 3; i++) {
			this.heads[i].yaw = (witherEntity.getHeadYaw(i - 1) - witherEntity.bodyYaw) * (float) (Math.PI / 180.0);
			this.heads[i].pitch = witherEntity.getHeadPitch(i - 1) * (float) (Math.PI / 180.0);
		}
	}
}
