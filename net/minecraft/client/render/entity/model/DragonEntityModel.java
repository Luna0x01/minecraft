...
public class DragonEntityModel extends EntityModel<EnderDragonEntity> {
	...
	public void method_17137(EnderDragonEntity enderDragonEntity, float f, float g, float h, float i, float j, float k) {
		...
		for (int w = 0; w < 2; w++) {
			...
			float x = l * (float) (Math.PI * 2);
			this.wing.pitch = 0.125F - (float)Math.cos((double)x) * 0.2F;
			this.wing.yaw = 0.25F;
			this.wing.roll = (float)(Math.sin((double)x) + 0.125) * 0.8F;
			this.wingTip.roll = -((float)(Math.sin((double)(x + 2.0F)) + 0.5)) * 0.75F;
		...
		float y = -((float)Math.sin((double)(l * (float) (Math.PI * 2)))) * 0.0F;
		t = l * (float) (Math.PI * 2);
		o = 10.0F;
		p = 60.0F;
		n = 0.0F;
		...
		for (int z = 0; z < 12; z++) {
			fs = enderDragonEntity.method_6817(12 + z, this.delta);
			y = (float)((double)y + Math.sin((double)((float)z * 0.45F + t)) * 0.05F);
			this.neck.yaw = (this.updateRotations(fs[0] - ds[0]) * 1.5F + 180.0F) * (float) (Math.PI / 180.0);
			this.neck.pitch = y + (float)(fs[1] - ds[1]) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
