...
public class BoatEntityModel extends CompositeEntityModel<BoatEntity> {
	...
	protected void setPaddleAngle(BoatEntity boatEntity, int i, float f) {
		...
		modelPart.pitch = (float)MathHelper.clampedLerp((float) (-Math.PI / 3), (float) (-Math.PI / 12), (double)((MathHelper.sin(-g) + 1.0F) / 2.0F));
		modelPart.yaw = (float)MathHelper.clampedLerp((float) (-Math.PI / 4), (float) (Math.PI / 4), (double)((MathHelper.sin(-g + 1.0F) + 1.0F) / 2.0F));
		if (i == 1) {
