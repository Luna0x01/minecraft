...
public class GameRenderer implements AutoCloseable, SynchronousResourceReloadListener {
	...
	private void bobViewWhenHurt(float f) {
		if (this.client.getCameraEntity() instanceof LivingEntity) {
			...
			float i = livingEntity.field_6271;
		...
		if (!this.client.skipGameRender) {
			...
			if (this.client.overlay != null) {
				...
				try {
					...
					CrashReport crashReport2 = CrashReport.create(var13, "Rendering screen");
		...
		if (f != 0.0F) {
			...
			for (int l = 0; l < k; l++) {
				...
				if (blockPos2.getY() <= blockPos.getY() + 10
					...
					double n = voxelShape.method_1102(Direction.Axis.field_11052, h, m);
					double o = (double)fluidState.getHeight(viewableWorld, blockPos2);
					double p;
					double q;
					if (n >= o) {
						p = n;
						q = voxelShape.method_1093(Direction.Axis.field_11052, h, m);
					} else {
						p = 0.0;
						q = 0.0;
					}
					...
					if (biome.getPrecipitation() != Biome.Precipitation.NONE) {
						...
						if (v != w) {
							...
							if (y >= 0.15F) {
								...
								double ah = (double)(-((float)(this.ticks & 511) + f) / 512.0F);
								double ai = this.random.nextDouble() + (double)o * 0.01 * (double)((float)this.random.nextGaussian());
								double aj = this.random.nextDouble() + (double)(o * (float)this.random.nextGaussian()) * 0.001;
								double ak = (double)((float)q + 0.5F) - this.camera.getPos().x;
								double al = (double)((float)p + 0.5F) - this.camera.getPos().z;
								float am = MathHelper.sqrt(ak * ak + al * al) / (float)m;
								float an = ((1.0F - am * am) * 0.3F + 0.5F) * g;
								mutable.set(q, x, p);
								int ao = (world.getLightmapIndex(mutable, 0) * 3 + 15728880) / 4;
								int ap = ao >> 16 & 65535;
								int aq = ao & 65535;
								bufferBuilder.vertex((double)q - s + 0.5, (double)w, (double)p - t + 0.5)
