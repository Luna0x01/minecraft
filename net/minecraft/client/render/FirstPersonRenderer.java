...
public class FirstPersonRenderer {
	...
	private void renderArms() {
		if (!this.client.player.isInvisible()) {
			...
			this.renderArm(Arm.field_6183);
			this.renderArm(Arm.field_6182);
			GlStateManager.popMatrix();
	...
	private void applyEatOrDrinkTransformation(float f, Arm arm, ItemStack itemStack) {
		...
		GlStateManager.translatef(j * 0.6F * (float)k, j * -0.5F, j * 0.0F);
		GlStateManager.rotatef((float)k * j * 90.0F, 0.0F, 1.0F, 0.0F);
		GlStateManager.rotatef(j * 10.0F, 1.0F, 0.0F, 0.0F);
		GlStateManager.rotatef((float)k * j * 30.0F, 0.0F, 0.0F, 1.0F);
	}
	...
	public void renderFirstPersonItem(float f) {
		...
		Hand hand = (Hand)MoreObjects.firstNonNull(abstractClientPlayerEntity.preferredHand, Hand.field_5808);
		...
		if (abstractClientPlayerEntity.isUsingItem()) {
			...
			ItemStack itemStack3 = abstractClientPlayerEntity.getMainHandStack();
			ItemStack itemStack4 = abstractClientPlayerEntity.getOffHandStack();
		...
		if (bl2) {
			float l = hand == Hand.field_5810 ? g : 0.0F;
			float m = 1.0F - MathHelper.lerp(f, this.prevEquipProgressOffHand, this.equipProgressOffHand);
			this.renderFirstPersonItem(abstractClientPlayerEntity, f, h, Hand.field_5810, l, this.offHand, m);
		}
	...
	public void renderFirstPersonItem(AbstractClientPlayerEntity abstractClientPlayerEntity, float f, float g, Hand hand, float h, ItemStack itemStack, float i) {
		...
		if (itemStack.isEmpty()) {
			...
			if (abstractClientPlayerEntity.isUsingItem() && abstractClientPlayerEntity.getItemUseTimeLeft() > 0 && abstractClientPlayerEntity.getActiveHand() == hand) {
				...
				GlStateManager.translatef((float)j * p, q, r);
				this.applyHandOffset(arm, i);
			...
			if (abstractClientPlayerEntity.isUsingItem() && abstractClientPlayerEntity.getItemUseTimeLeft() > 0 && abstractClientPlayerEntity.getActiveHand() == hand) {
				...
				switch (itemStack.getUseAction()) {
					...
					case field_8953:
						this.applyHandOffset(arm, i);
						GlStateManager.translatef((float)s * -0.2785682F, 0.18344387F, 0.15731531F);
						GlStateManager.rotatef(-13.935F, 1.0F, 0.0F, 0.0F);
						GlStateManager.rotatef((float)s * 35.3F, 0.0F, 1.0F, 0.0F);
						GlStateManager.rotatef((float)s * -9.785F, 0.0F, 0.0F, 1.0F);
						float t = (float)itemStack.getMaxUseTime() - ((float)this.client.player.getItemUseTimeLeft() - f + 1.0F);
						float u = t / 20.0F;
						u = (u * u + u * 2.0F) / 3.0F;
						if (u > 1.0F) {
							u = 1.0F;
						...
						if (u > 0.1F) {
							float v = MathHelper.sin((t - 0.1F) * 1.3F);
							float w = u - 0.1F;
							float x = v * w;
							GlStateManager.translatef(x * 0.0F, x * 0.004F, x * 0.0F);
						}

						GlStateManager.translatef(u * 0.0F, u * 0.0F, u * 0.04F);
						GlStateManager.scalef(1.0F, 1.0F, 1.0F + u * 0.2F);
						GlStateManager.rotatef((float)s * 45.0F, 0.0F, -1.0F, 0.0F);
						break;
					case field_8951:
						this.applyHandOffset(arm, i);
						GlStateManager.translatef((float)s * -0.5F, 0.7F, 0.1F);
						GlStateManager.rotatef(-55.0F, 1.0F, 0.0F, 0.0F);
						GlStateManager.rotatef((float)s * 35.3F, 0.0F, 1.0F, 0.0F);
						GlStateManager.rotatef((float)s * -9.785F, 0.0F, 0.0F, 1.0F);
						float y = (float)itemStack.getMaxUseTime() - ((float)this.client.player.getItemUseTimeLeft() - f + 1.0F);
						float z = y / 10.0F;
						if (z > 1.0F) {
							z = 1.0F;
						}
						...
						if (z > 0.1F) {
							float aa = MathHelper.sin((y - 0.1F) * 1.3F);
							float ab = z - 0.1F;
							float ac = aa * ab;
							GlStateManager.translatef(ac * 0.0F, ac * 0.004F, ac * 0.0F);
						}

						GlStateManager.translatef(0.0F, 0.0F, z * 0.2F);
						GlStateManager.scalef(1.0F, 1.0F, 1.0F + z * 0.2F);
						GlStateManager.rotatef((float)s * 45.0F, 0.0F, -1.0F, 0.0F);
				}
			} else if (abstractClientPlayerEntity.isUsingRiptide()) {
				this.applyHandOffset(arm, i);
				int ad = bl4 ? 1 : -1;
				GlStateManager.translatef((float)ad * -0.4F, 0.8F, 0.3F);
				GlStateManager.rotatef((float)ad * 65.0F, 0.0F, 1.0F, 0.0F);
				GlStateManager.rotatef((float)ad * -85.0F, 0.0F, 0.0F, 1.0F);
			} else {
				float ae = -0.4F * MathHelper.sin(MathHelper.sqrt(h) * (float) Math.PI);
				float af = 0.2F * MathHelper.sin(MathHelper.sqrt(h) * (float) (Math.PI * 2));
				float ag = -0.2F * MathHelper.sin(h * (float) Math.PI);
				int ah = bl4 ? 1 : -1;
				GlStateManager.translatef((float)ah * ae, af, ag);
				this.applyHandOffset(arm, i);
				this.method_3217(arm, h);
			}

			this.renderItemFromSide(abstractClientPlayerEntity, itemStack, bl4 ? ModelTransformation.Type.field_4322 : ModelTransformation.Type.field_4321, !bl4);
		}
