...
public class UnicodeTextureFont implements Font {
	...
	public UnicodeTextureFont(ResourceManager resourceManager, byte[] bs, String string) {
		...
		for (int i = 0; i < 256; i++) {
			...
			try {
				...
				try (NativeImage nativeImage = NativeImage.read(NativeImage.Format.field_4997, resource.getInputStream())) {
	...
	private NativeImage getGlyphImage(Identifier identifier) {
		try {
			...
			NativeImage var4;
			try {
				var4 = NativeImage.read(NativeImage.Format.field_4997, resource.getInputStream());
