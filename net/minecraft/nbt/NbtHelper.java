...
public final class NbtHelper {
			...
			if (listTag.isEmpty()) {
				...
				for (int i = 0; i < listTag.size(); i++) {
					...
					for (int j = 0; j < listTag2.size(); j++) {
						if (matches(tag4, listTag2.get(j), bl)) {
							bl2 = true;
							break;
						}
					}
	...
	public static BlockState toBlockState(CompoundTag compoundTag) {
		if (!compoundTag.contains("Name", 8)) {
			...
			Block block = Registry.field_11146.get(new Identifier(compoundTag.getString("Name")));
	...
	public static CompoundTag fromBlockState(BlockState blockState) {
		CompoundTag compoundTag = new CompoundTag();
		compoundTag.putString("Name", Registry.field_11146.getId(blockState.getBlock()).toString());
