...
public class NbtOps implements DynamicOps<Tag> {
	...
	public DataResult<Consumer<BiConsumer<Tag, Tag>>> getMapEntries(Tag tag) {
		if (!(tag instanceof CompoundTag)) {
			...
			return DataResult.success(
				(Consumer)biConsumer -> compoundTag.getKeys().forEach(string -> biConsumer.accept(this.createString(string), compoundTag.get(string)))
			);
		}
	}
	...
	public DataResult<ByteBuffer> getByteBuffer(Tag tag) {
		return tag instanceof ByteArrayTag ? DataResult.success(ByteBuffer.wrap(((ByteArrayTag)tag).getByteArray())) : super.getByteBuffer(tag);
	}
	...
	public DataResult<IntStream> getIntStream(Tag tag) {
		return tag instanceof IntArrayTag ? DataResult.success(Arrays.stream(((IntArrayTag)tag).getIntArray())) : super.getIntStream(tag);
	}
	...
	public DataResult<LongStream> getLongStream(Tag tag) {
		return tag instanceof LongArrayTag ? DataResult.success(Arrays.stream(((LongArrayTag)tag).getLongArray())) : super.getLongStream(tag);
	}
	...
	public Tag createList(Stream<Tag> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (tag instanceof ByteTag) {
				List<Byte> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((ByteTag)tagx).getByte()));
				return new ByteArrayTag(list);
			} else if (tag instanceof IntTag) {
				List<Integer> list2 = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((IntTag)tagx).getInt()));
				return new IntArrayTag(list2);
			} else if (tag instanceof LongTag) {
				List<Long> list3 = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((LongTag)tagx).getLong()));
