...
public interface CrossSamplingLayer extends ParentedLayer, NorthWestCoordinateTransformer {
	...
	default int sample(LayerSampleContext<?> layerSampleContext, LayerSampler layerSampler, int i, int j) {
		return this.sample(
			layerSampleContext,
			layerSampler.sample(this.transformX(i + 1), this.transformZ(j + 0)),
			layerSampler.sample(this.transformX(i + 2), this.transformZ(j + 1)),
			layerSampler.sample(this.transformX(i + 1), this.transformZ(j + 2)),
			layerSampler.sample(this.transformX(i + 0), this.transformZ(j + 1)),
