...
public class RandomFeatureConfig implements FeatureConfig {
	...
	public <T> Dynamic<T> serialize(DynamicOps<T> dynamicOps) {
		T object = (T)dynamicOps.createList(this.features.stream().map(randomFeatureEntry -> randomFeatureEntry.serialize(dynamicOps).getValue()));
		T object2 = (T)this.defaultFeature.serialize(dynamicOps).getValue();
		return new Dynamic(
