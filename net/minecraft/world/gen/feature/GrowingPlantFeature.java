		...
		for (int j = 1; j <= i; j++) {
			...
			if (blockState2.isAir() || growingPlantFeatureConfig.allowWater && blockState2.getFluidState().isIn(FluidTags.WATER)) {
				if (j == i || !blockState.isAir()) {
					worldAccess.setBlockState(mutable, growingPlantFeatureConfig.headProvider.getBlockState(random, mutable), 2);
					break;
				}

				worldAccess.setBlockState(mutable, growingPlantFeatureConfig.bodyProvider.getBlockState(random, mutable), 2);
			}
