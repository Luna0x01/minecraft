...
public class RandomPatchFeature extends Feature<RandomPatchFeatureConfig> {
	...
	public boolean generate(
		...
		if (randomPatchFeatureConfig.project) {
			blockPos2 = iWorld.getTopPosition(Heightmap.Type.field_13194, blockPos);
		} else {
			blockPos2 = blockPos;
		}
			...
			if ((iWorld.isAir(mutable) || randomPatchFeatureConfig.canReplace && iWorld.getBlockState(mutable).getMaterial().isReplaceable())
				...
				&& (
					!randomPatchFeatureConfig.needsWater
						|| iWorld.getFluidState(blockPos4.west()).matches(FluidTags.field_15517)
						|| iWorld.getFluidState(blockPos4.east()).matches(FluidTags.field_15517)
						|| iWorld.getFluidState(blockPos4.north()).matches(FluidTags.field_15517)
						|| iWorld.getFluidState(blockPos4.south()).matches(FluidTags.field_15517)
				)) {
