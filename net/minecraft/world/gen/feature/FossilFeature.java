	...
	public boolean generate(
		...
		for (int m = 0; m < blockPos2.getX(); m++) {
			for (int n = 0; n < blockPos2.getZ(); n++) {
				l = Math.min(l, iWorld.getTopY(Heightmap.Type.field_13195, blockPos.getX() + m + j, blockPos.getZ() + n + k));
			}
		}

		int o = Math.max(l - 15 - random2.nextInt(10), 10);
		BlockPos blockPos3 = structure.method_15167(blockPos.add(j, o, k), BlockMirror.field_11302, blockRotation);
