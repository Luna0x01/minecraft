...
public final class ChunkSkyLightProvider extends ChunkLightProvider<SkyLightStorage.Data, SkyLightStorage> {
			...
			if (i >= 15) {
				...
				if (atomicInteger.get() >= 15) {
					...
					if (direction != null) {
						...
						VoxelShape voxelShape3 = this.method_20710(blockState2, l, Direction.field_11033);
						if (VoxelShapes.method_20713(voxelShape3, VoxelShapes.empty())) {
							return 15;
						}

						int u = bl ? -1 : 0;
						Direction direction3 = Direction.fromVector(r, u, t);
						if (direction3 == null) {
							return 15;
						}

						VoxelShape voxelShape4 = this.method_20710(blockState, m, direction3.getOpposite());
						if (VoxelShapes.method_20713(VoxelShapes.empty(), voxelShape4)) {
							return 15;
						}
					}
	...
	protected void updateNeighborsRecursively(long l, int i, boolean bl) {
		...
		long r = BlockPos.add(l, 0, -1 - o * 16, 0);
		long s = ChunkSectionPos.toChunkLong(r);
		if (m == s || this.lightStorage.hasChunk(s)) {
			this.updateRecursively(l, r, i, bl);
		}

		long t = BlockPos.offset(l, Direction.field_11036);
		long u = ChunkSectionPos.toChunkLong(t);
		if (m == u || this.lightStorage.hasChunk(u)) {
			this.updateRecursively(l, t, i, bl);
		}
		...
		for (Direction direction : HORIZONTAL_DIRECTIONS) {
			...
			do {
				long w = BlockPos.add(l, direction.getOffsetX(), -v, direction.getOffsetZ());
				long x = ChunkSectionPos.toChunkLong(w);
				if (m == x) {
					this.updateRecursively(l, w, i, bl);
					break;
				}

				if (this.lightStorage.hasChunk(x)) {
					this.updateRecursively(l, w, i, bl);
				}
			} while (++v > o * 16);
		}
	...
	protected int getMergedLevel(long l, long m, int i) {
		...
		for (Direction direction : DIRECTIONS_SKYLIGHT) {
			...
			if (chunkNibbleArray2 != null) {
				...
				ChunkNibbleArray chunkNibbleArray4 = this.lightStorage.getDataForChunk(p, true);
				if (o != m) {
					int r;
					if (chunkNibbleArray4 != null) {
						r = this.getPropagatedLevel(o, l, this.getCurrentLevelFromArray(chunkNibbleArray4, o));
					} else {
