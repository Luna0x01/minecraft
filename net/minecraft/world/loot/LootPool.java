...
public class LootPool {
	...
	public void check(LootTableReporter lootTableReporter, Function<Identifier, LootSupplier> function, Set<Identifier> set, LootContextType lootContextType) {
		...
		for (int j = 0; j < this.functions.length; j++) {
			this.functions[j].check(lootTableReporter.makeChild(".functions[" + j + "]"), function, set, lootContextType);
		}

		for (int k = 0; k < this.entries.length; k++) {
			this.entries[k].check(lootTableReporter.makeChild(".entries[" + k + "]"), function, set, lootContextType);
		}
	}
