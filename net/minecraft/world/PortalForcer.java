...
public class PortalForcer {
	...
	private final Map<ColumnPos, PortalForcer.TicketInfo> ticketInfos = Maps.newHashMapWithExpectedSize(4096);
		...
		if (!bl && this.activePortals.containsKey(columnPos)) {
			...
			if (blockPos2 == null) {
				...
				if (bl2) {
					...
					LOGGER.debug("Adding nether portal ticket for {}:{}", new Supplier[]{this.world.getDimension()::getType, () -> columnPos});
	...
	public boolean createPortal(Entity entity) {
		...
		BlockState blockState = PORTAL_BLOCK.getDefaultState().with(PortalBlock.AXIS, aw == 0 ? Direction.Axis.field_11051 : Direction.Axis.field_11048);
	...
	private void removeOldTickets(long l) {
		...
		while (iterator.hasNext()) {
			...
			if (ticketInfo.lastUsedTime < m) {
				...
				LOGGER.debug("Removing nether portal ticket for {}:{}", new Supplier[]{this.world.getDimension()::getType, () -> columnPos});
