...
public class DecoderHandler extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
		if (byteBuf.readableBytes() != 0) {
			...
			Packet<?> packet = ((NetworkState)channelHandlerContext.channel().attr(ClientConnection.PROTOCOL_ATTRIBUTE_KEY).get())
				.getPacketHandler(this.side, i, packetByteBuf);
			if (packet == null) {
				throw new IOException("Bad packet id " + i);
			} else if (packetByteBuf.readableBytes() > 0) {
				throw new IOException(
					"Packet "
						+ ((NetworkState)channelHandlerContext.channel().attr(ClientConnection.PROTOCOL_ATTRIBUTE_KEY).get()).getId()
						+ "/"
						+ i
						+ " ("
