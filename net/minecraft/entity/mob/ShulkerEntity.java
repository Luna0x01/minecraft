...
public class ShulkerEntity extends GolemEntity implements Monster {
	...
	protected void initDataTracker() {
		...
		this.dataTracker.startTracking(ATTACHED_FACE, Direction.field_11033);
				...
				if (!list.isEmpty()) {
					for (Entity entity : list) {
						if (!(entity instanceof ShulkerEntity) && !entity.noClip) {
							entity.move(
								MovementType.field_6309, new Vec3d(h * (double)direction4.getOffsetX(), h * (double)direction4.getOffsetY(), h * (double)direction4.getOffsetZ())
							);
						}
					}
				}
	...
	class SearchForPlayerGoal extends FollowTargetGoal<PlayerEntity> {
		...
		@Override
		public boolean canStart() {
			return ShulkerEntity.this.world.getDifficulty() == Difficulty.field_5801 ? false : super.canStart();
		}
		...
		protected Box getSearchBox(double d) {
			...
			if (direction.getAxis() == Direction.Axis.field_11048) {
				...
				return direction.getAxis() == Direction.Axis.field_11051 ? this.mob.getBoundingBox().expand(d, d, 4.0) : this.mob.getBoundingBox().expand(d, 4.0, d);
			}
		}
	}
	...
	class ShootBulletGoal extends Goal {
		...
		public boolean canStart() {
			...
			return livingEntity != null && livingEntity.isAlive() ? ShulkerEntity.this.world.getDifficulty() != Difficulty.field_5801 : false;
		}
		...
		@Override
		public void tick() {
			if (ShulkerEntity.this.world.getDifficulty() != Difficulty.field_5801) {
				this.counter--;
				LivingEntity livingEntity = ShulkerEntity.this.getTarget();
