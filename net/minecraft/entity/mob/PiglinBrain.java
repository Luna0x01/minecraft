...
public class PiglinBrain {
	...
	private static void addAdmireItemActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
			...
			10,
			ImmutableList.of(
				new WalkToNearestVisibleWantedItemTask(PiglinBrain::doesNotHaveGoldInOffHand, 1.0F, true, 9), new WantNewItemTask(9), new AdmireItemTimeLimitTask(200, 200)
			),
	...
	private static Optional<? extends LivingEntity> getPreferredTarget(PiglinEntity piglin) {
		...
		if (getNearestZombifiedPiglin(piglin)) {
			...
			if (optional.isPresent() && Sensor.testAttackableTargetPredicateIgnoreVisibility(piglin, (LivingEntity)optional.get())) {
				...
				Optional<MobEntity> optional3 = brain.getOptionalMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
				if (optional3.isPresent()) {
					return optional3;
				} else {
					Optional<PlayerEntity> optional4 = brain.getOptionalMemory(MemoryModuleType.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GOLD);
					return optional4.isPresent() && Sensor.testAttackableTargetPredicate(piglin, (LivingEntity)optional4.get()) ? optional4 : Optional.empty();
				}
			}
		}
	}
