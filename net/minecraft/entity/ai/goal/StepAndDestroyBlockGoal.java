...
public class StepAndDestroyBlockGoal extends MoveToTargetPosGoal {
	...
	public void tick() {
		...
		if (this.hasReached() && blockPos2 != null) {
			...
			if (this.counter % 2 == 0) {
				Vec3d vec3d2 = this.stepAndDestroyMob.getVelocity();
				this.stepAndDestroyMob.setVelocity(vec3d2.x, -0.3, vec3d2.z);
				if (this.counter % 6 == 0) {
			...
			if (this.counter > 60) {
				...
				if (!world.isClient) {
					for (int i = 0; i < 20; i++) {
						...
						((ServerWorld)world)
							.spawnParticles(ParticleTypes.field_11203, (double)blockPos2.getX() + 0.5, (double)blockPos2.getY(), (double)blockPos2.getZ() + 0.5, 1, e, f, g, 0.15F);
					}
	...
	protected boolean isTargetPos(ViewableWorld viewableWorld, BlockPos blockPos) {
		Chunk chunk = viewableWorld.getChunk(blockPos.getX() >> 4, blockPos.getZ() >> 4, ChunkStatus.field_12803, false);
		return chunk == null
			? false
