...
public class Brain<E extends LivingEntity> {
	...
	public static <E extends LivingEntity> Codec<Brain<E>> createBrainCodec(
		...
		mutableObject.setValue(
			(new MapCodec<Brain<E>>() {
					...
					public <T> DataResult<Brain<E>> decode(DynamicOps<T> dynamicOps, MapLike<T> mapLike) {
						...
						ImmutableList<Brain.MemoryEntry<?>> immutableList = (ImmutableList<Brain.MemoryEntry<?>>)((DataResult)mutableObject.getValue())
		...
		return (Codec<Brain<E>>)mutableObject.getValue();
	}
	...
	static final class MemoryEntry<U> {
		...
		public <T> void serialize(DynamicOps<T> ops, RecordBuilder<T> builder) {
			this.type
				.getCodec()
				.ifPresent(codec -> this.data.ifPresent(memory -> builder.add(Registry.MEMORY_MODULE_TYPE.encodeStart(ops, this.type), codec.encodeStart(ops, memory))));
		}
	}
