...
public class GoToPointOfInterestTask extends Task<VillagerEntity> {
	...
	protected void run(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
		...
		for (int j = 0; j < 5; j++) {
			Vec3d vec3d2 = TargetFinder.findGroundTarget(
		...
		if (vec3d != null) {
			villagerEntity.getBrain().putMemory(MemoryModuleType.field_18445, new WalkTarget(vec3d, this.speed, this.completionRange));
		}
	}
}
