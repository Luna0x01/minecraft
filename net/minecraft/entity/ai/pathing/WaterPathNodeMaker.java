...
public class WaterPathNodeMaker extends PathNodeMaker {
	...
	private PathNode getPathNodeInWater(int i, int j, int k) {
		...
		return (!this.field_58 || pathNodeType != PathNodeType.field_16) && pathNodeType != PathNodeType.field_18 ? null : this.getNode(i, j, k);
	}
	...
	private PathNodeType getNodeType(int i, int j, int k) {
		...
		for (int l = i; l < i + this.field_31; l++) {
			for (int m = j; m < j + this.field_30; m++) {
				for (int n = k; n < k + this.field_28; n++) {
					...
					if (fluidState.isEmpty() && blockState.canPlaceAtSide(this.field_20622, mutable.down(), BlockPlacementEnvironment.field_48) && blockState.isAir()) {
						return PathNodeType.field_16;
					}
		...
		return blockState2.canPlaceAtSide(this.field_20622, mutable, BlockPlacementEnvironment.field_48) ? PathNodeType.field_18 : PathNodeType.field_22;
	}
}
