...
public class AmphibiousPathNodeMaker extends LandPathNodeMaker {
	...
	public int getSuccessors(PathNode[] successors, PathNode node) {
		...
		if (bl && bl3) {
			PathNode pathNode8 = this.getPathNode(node.x + 1, node.y, node.z - 1, 1, d);
		...
		if (bl2 && bl4) {
			PathNode pathNode9 = this.getPathNode(node.x - 1, node.y, node.z + 1, 1, d);
		...
		if (bl2 && bl3) {
			PathNode pathNode10 = this.getPathNode(node.x + 1, node.y, node.z + 1, 1, d);
