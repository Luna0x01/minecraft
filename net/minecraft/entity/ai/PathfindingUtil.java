	...
	private static Vec3d findTarget(
		...
		for (int k = 0; k < 10; k++) {
			...
			if (blockPos2 != null) {
				...
				BlockPos blockPos4 = new BlockPos((double)l + mobEntityWithAi.x, (double)m + mobEntityWithAi.y, (double)n + mobEntityWithAi.z);
				if ((!bl2 || mobEntityWithAi.isInWalkTargetRange(blockPos4)) && entityNavigation.isValidPosition(blockPos4)) {
					if (!bl) {
						blockPos4 = method_6372(blockPos4, mobEntityWithAi);
						if (isWater(blockPos4, mobEntityWithAi)) {
							continue;
						}
					}
