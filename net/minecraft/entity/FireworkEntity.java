...
public class FireworkEntity extends Entity implements FlyingItemEntity, Projectile {
	...
	public void tick() {
		...
		HitResult hitResult = ProjectileUtil.getCollision(
			this,
			this.getBoundingBox().stretch(vec3d3).expand(1.0),
		...
		float f = MathHelper.sqrt(squaredHorizontalLength(vec3d3));
		this.yaw = (float)(MathHelper.atan2(vec3d3.x, vec3d3.z) * 180.0F / (float)Math.PI);
		this.pitch = (float)(MathHelper.atan2(vec3d3.y, (double)f) * 180.0F / (float)Math.PI);
