...
public class EnderDragonFight {
	...
	public void tick() {
		...
		if (!this.bossBar.getPlayers().isEmpty()) {
			this.world.getChunkManager().addTicket(ChunkTicketType.field_17264, new ChunkPos(0, 0), 9, Unit.field_17274);
			...
			this.world.getChunkManager().removeTicket(ChunkTicketType.field_17264, new ChunkPos(0, 0), 9, Unit.field_17274);
		}
	}
	...
	private BlockPattern.Result findEndPortal() {
		...
		int k = this.world.getTopPosition(Heightmap.Type.field_13197, EndPortalFeature.ORIGIN).getY();

		for (int l = k; l >= 0; l--) {
			BlockPattern.Result result2 = this.endPortalPattern
				.searchAround(this.world, new BlockPos(EndPortalFeature.ORIGIN.getX(), l, EndPortalFeature.ORIGIN.getZ()));
			if (result2 != null) {
				if (this.exitPortalLocation == null) {
	...
	public void respawnDragon() {
		if (this.dragonKilled && this.dragonSpawnState == null) {
			...
			List<EnderCrystalEntity> list = Lists.newArrayList();
