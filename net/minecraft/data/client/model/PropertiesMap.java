...
public final class PropertiesMap {
	...
	public PropertiesMap method_25819(Property.Value<?> value) {
		return new PropertiesMap(ImmutableList.builder().addAll(this.propertyValues).add(value).build());
	}

	public PropertiesMap with(PropertiesMap propertiesMap) {
		return new PropertiesMap(ImmutableList.builder().addAll(this.propertyValues).addAll(propertiesMap.propertyValues).build());
	}
