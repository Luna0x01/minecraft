...
public class class_3408 extends DataFix {
	private static final Set<String> field_16608 = ImmutableSet.builder()
		...
		.build();
	private static final Map<String, String> field_16609 = ImmutableMap.builder()
		...
		.build();
	private static final Map<String, String> field_16610 = ImmutableMap.builder()
		...
		.build();
	private static final Map<String, String> field_16611 = ImmutableMap.builder()
		.put("stat.entityKilledBy", "minecraft:killed_by")
		.put("stat.killEntity", "minecraft:killed")
		.build();
	private static final Map<String, String> field_16612 = ImmutableMap.builder()
	...
	public class_3408(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
			"StatsCounterFix",
			this.getInputSchema().getType(class_3402.field_16588),
			type,
			typed -> {
				...
				if (optional.isPresent()) {
					for (Entry<? extends Dynamic<?>, ? extends Dynamic<?>> entry : ((Map)optional.get()).entrySet()) {
						if (((Dynamic)entry.getValue()).getNumberValue().isPresent()) {
							...
							if (!field_16608.contains(string)) {
								String string2;
								String string3;
								if (field_16609.containsKey(string)) {
									string2 = "minecraft:custom";
									string3 = (String)field_16609.get(string);
								} else {
									...
									if ("stat.mineBlock".equals(string4)) {
										...
										string3 = this.method_15262(string.substring(i + 1).replace('.', ':'));
									} else if (field_16610.containsKey(string4)) {
										string2 = (String)field_16610.get(string4);
										String string8 = string.substring(i + 1).replace('.', ':');
										String string9 = this.method_15261(string8);
										string3 = string9 == null ? string8 : string9;
									} else {
										if (!field_16611.containsKey(string4)) {
											continue;
										}

										string2 = (String)field_16611.get(string4);
										String string12 = string.substring(i + 1).replace('.', ':');
										string3 = (String)field_16612.getOrDefault(string12, string12);
									}
								}
				...
				return (Typed)((Optional)type.readTyped(dynamic.emptyMap().set("stats", dynamic.createMap(map))).getSecond())
	...
	@Nullable
	protected String method_15261(String string) {
		return class_3389.method_15138(string, 0);
	}

	protected String method_15262(String string) {
		return class_4500.method_21604(string);
	}
}
