...
public class StrongholdGenerator {
	...
	private static StrongholdGenerator.Piece createPiece(
		...
		if (pieceType == StrongholdGenerator.Corridor.class) {
			piece = StrongholdGenerator.Corridor.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.PrisonHall.class) {
			piece = StrongholdGenerator.PrisonHall.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.LeftTurn.class) {
			piece = StrongholdGenerator.LeftTurn.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.RightTurn.class) {
			piece = StrongholdGenerator.RightTurn.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.SquareRoom.class) {
			piece = StrongholdGenerator.SquareRoom.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.Stairs.class) {
			piece = StrongholdGenerator.Stairs.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.SpiralStaircase.class) {
			piece = StrongholdGenerator.SpiralStaircase.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.FiveWayCrossing.class) {
			piece = StrongholdGenerator.FiveWayCrossing.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.ChestCorridor.class) {
			piece = StrongholdGenerator.ChestCorridor.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.Library.class) {
			piece = StrongholdGenerator.Library.create(structurePiecesHolder, random, x, y, z, orientation, chainLength);
		} else if (pieceType == StrongholdGenerator.PortalRoom.class) {
			piece = StrongholdGenerator.PortalRoom.create(structurePiecesHolder, x, y, z, orientation, chainLength);
		}
	...
	private static StrongholdGenerator.Piece pickPiece(
		StrongholdGenerator.Start start, StructurePiecesHolder structurePiecesHolder, Random random, int x, int y, int z, Direction orientation, int chainLength
	) {
		if (!checkRemainingPieces()) {
			return null;
		} else {
			if (activePieceType != null) {
				StrongholdGenerator.Piece piece = createPiece(activePieceType, structurePiecesHolder, random, x, y, z, orientation, chainLength);
				activePieceType = null;
				if (piece != null) {
					return piece;
				}
			...
			while (i < 5) {
				...
				for (StrongholdGenerator.PieceData pieceData : possiblePieces) {
					...
					if (j < 0) {
						...
						StrongholdGenerator.Piece piece2 = createPiece(pieceData.pieceType, structurePiecesHolder, random, x, y, z, orientation, chainLength);
						if (piece2 != null) {
			...
			BlockBox blockBox = StrongholdGenerator.SmallCorridor.create(structurePiecesHolder, random, x, y, z, orientation);
	...
	static StructurePiece pieceGenerator(
		...
		if (chainLength > 50) {
			...
			StructurePiece structurePiece = pickPiece(start, structurePiecesHolder, random, x, y, z, orientation, chainLength + 1);
	...
	public static class ChestCorridor extends StrongholdGenerator.Piece {
		...
		public void fillOpenings(StructurePiece start, StructurePiecesHolder structurePiecesHolder, Random random) {
			this.fillForwardOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 1);
		}
	...
	public static class Corridor extends StrongholdGenerator.Piece {
		...
		public void fillOpenings(StructurePiece start, StructurePiecesHolder structurePiecesHolder, Random random) {
			...
			if (this.leftExitExists) {
				this.fillNWOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 2);
			}

			if (this.rightExitExists) {
				this.fillSEOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 2);
			}
		}
	...
	public static class FiveWayCrossing extends StrongholdGenerator.Piece {
		...
		public void fillOpenings(StructurePiece start, StructurePiecesHolder structurePiecesHolder, Random random) {
			...
			this.fillForwardOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 5, 1);
			if (this.lowerLeftExists) {
				this.fillNWOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, i, 1);
			}

			if (this.upperLeftExists) {
				this.fillNWOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, j, 7);
			}

			if (this.lowerRightExists) {
				this.fillSEOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, i, 1);
			}

			if (this.upperRightExists) {
				this.fillSEOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, j, 7);
			}
		}
	...
	public static class LeftTurn extends StrongholdGenerator.Turn {
		...
		public void fillOpenings(StructurePiece start, StructurePiecesHolder structurePiecesHolder, Random random) {
			...
			if (direction != Direction.NORTH && direction != Direction.EAST) {
				this.fillSEOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 1);
			} else {
				this.fillNWOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 1);
			}
		}
	...
	public static class Library extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.Library create(
			StructurePiecesHolder structurePiecesHolder, Random random, int x, int y, int z, Direction orientation, int chainLength
		) {
			BlockBox blockBox = BlockBox.rotated(x, y, z, -4, -1, 0, 14, 11, 15, orientation);
			if (!isInBounds(blockBox) || structurePiecesHolder.getIntersecting(blockBox) != null) {
		...
		public boolean generate(
			...
			this.fillWithOutlineUnderSeaLevel(
				world, boundingBox, random, 0.07F, 2, 1, 1, 11, 4, 13, Blocks.COBWEB.getDefaultState(), Blocks.COBWEB.getDefaultState(), false, false
			);
			int j = 1;
			int k = 12;
			...
			for (int l = 1; l <= 13; l++) {
				if ((l - 1) % 4 == 0) {
					...
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.EAST), 2, 3, l, boundingBox);
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.WEST), 11, 3, l, boundingBox);
					if (this.tall) {
			...
			if (this.tall) {
				...
				this.addBlock(
					world, Blocks.OAK_FENCE.getDefaultState().with(FenceBlock.NORTH, Boolean.valueOf(true)).with(FenceBlock.EAST, Boolean.valueOf(true)), 3, 6, 2, boundingBox
				);
				this.addBlock(
					world,
				...
				this.addBlock(world, blockState4.with(FenceBlock.NORTH, Boolean.valueOf(true)), 6, 7, 6, boundingBox);
				this.addBlock(world, blockState4.with(FenceBlock.SOUTH, Boolean.valueOf(true)), 6, 7, 8, boundingBox);
				this.addBlock(world, blockState5.with(FenceBlock.NORTH, Boolean.valueOf(true)), 7, 7, 6, boundingBox);
				this.addBlock(world, blockState5.with(FenceBlock.SOUTH, Boolean.valueOf(true)), 7, 7, 8, boundingBox);
	...
	abstract static class Piece extends StructurePiece {
		...
		protected StructurePiece fillForwardOpening(
			StrongholdGenerator.Start start, StructurePiecesHolder structurePiecesHolder, Random random, int leftRightOffset, int heightOffset
		) {
			Direction direction = this.getFacing();
			if (direction != null) {
				switch (direction) {
		...
		protected StructurePiece fillNWOpening(
			StrongholdGenerator.Start start, StructurePiecesHolder structurePiecesHolder, Random random, int heightOffset, int leftRightOffset
		) {
			Direction direction = this.getFacing();
			if (direction != null) {
				switch (direction) {
	...
	public static class PortalRoom extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.PortalRoom create(StructurePiecesHolder structurePiecesHolder, int x, int y, int z, Direction orientation, int chainLength) {
		...
		public boolean generate(
			...
			this.addBlock(world, blockState4.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[0])), 4, 3, 8, boundingBox);
			this.addBlock(world, blockState4.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[1])), 5, 3, 8, boundingBox);
			this.addBlock(world, blockState4.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[2])), 6, 3, 8, boundingBox);
			this.addBlock(world, blockState5.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[3])), 4, 3, 12, boundingBox);
			this.addBlock(world, blockState5.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[4])), 5, 3, 12, boundingBox);
			this.addBlock(world, blockState5.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[5])), 6, 3, 12, boundingBox);
			this.addBlock(world, blockState6.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[6])), 3, 3, 9, boundingBox);
			this.addBlock(world, blockState6.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[7])), 3, 3, 10, boundingBox);
			this.addBlock(world, blockState6.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[8])), 3, 3, 11, boundingBox);
			this.addBlock(world, blockState7.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[9])), 7, 3, 9, boundingBox);
			this.addBlock(world, blockState7.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[10])), 7, 3, 10, boundingBox);
			this.addBlock(world, blockState7.with(EndPortalFrameBlock.EYE, Boolean.valueOf(bls[11])), 7, 3, 11, boundingBox);
			if (bl) {
	...
	public static class PrisonHall extends StrongholdGenerator.Piece {
		...
		public boolean generate(
			...
			for (int i = 1; i <= 3; i++) {
				this.addBlock(
					world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.NORTH, Boolean.valueOf(true)).with(PaneBlock.SOUTH, Boolean.valueOf(true)), 4, i, 4, boundingBox
				);
				this.addBlock(
					world,
					Blocks.IRON_BARS
				...
				this.addBlock(
					world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.NORTH, Boolean.valueOf(true)).with(PaneBlock.SOUTH, Boolean.valueOf(true)), 4, i, 6, boundingBox
				);
				this.addBlock(
					world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.WEST, Boolean.valueOf(true)).with(PaneBlock.EAST, Boolean.valueOf(true)), 5, i, 5, boundingBox
				);
				this.addBlock(
					world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.WEST, Boolean.valueOf(true)).with(PaneBlock.EAST, Boolean.valueOf(true)), 6, i, 5, boundingBox
				);
				this.addBlock(
					world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.WEST, Boolean.valueOf(true)).with(PaneBlock.EAST, Boolean.valueOf(true)), 7, i, 5, boundingBox
				);
			}

			this.addBlock(
				world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.NORTH, Boolean.valueOf(true)).with(PaneBlock.SOUTH, Boolean.valueOf(true)), 4, 3, 2, boundingBox
			);
			this.addBlock(
				world, Blocks.IRON_BARS.getDefaultState().with(PaneBlock.NORTH, Boolean.valueOf(true)).with(PaneBlock.SOUTH, Boolean.valueOf(true)), 4, 3, 8, boundingBox
			);
	...
	public static class SquareRoom extends StrongholdGenerator.Piece {
		...
		@Override
		public void fillOpenings(StructurePiece start, StructurePiecesHolder structurePiecesHolder, Random random) {
			this.fillForwardOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 4, 1);
			this.fillNWOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 4);
			this.fillSEOpening((StrongholdGenerator.Start)start, structurePiecesHolder, random, 1, 4);
		}
		...
		public boolean generate(
			...
			switch (this.roomType) {
				case 0:
					...
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.WEST), 4, 3, 5, boundingBox);
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.EAST), 6, 3, 5, boundingBox);
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.SOUTH), 5, 3, 4, boundingBox);
					this.addBlock(world, Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.NORTH), 5, 3, 6, boundingBox);
				...
				case 2:
					for (int j = 1; j <= 9; j++) {
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 1, 3, j, boundingBox);
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 9, 3, j, boundingBox);
					}

					for (int k = 1; k <= 9; k++) {
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), k, 3, 1, boundingBox);
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), k, 3, 9, boundingBox);
					}
					...
					for (int l = 1; l <= 3; l++) {
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 4, l, 4, boundingBox);
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 6, l, 4, boundingBox);
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 4, l, 6, boundingBox);
						this.addBlock(world, Blocks.COBBLESTONE.getDefaultState(), 6, l, 6, boundingBox);
					}
	...
	public static class Stairs extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.Stairs create(
			StructurePiecesHolder structurePiecesHolder, Random random, int x, int y, int z, Direction orientation, int chainLength
		) {
