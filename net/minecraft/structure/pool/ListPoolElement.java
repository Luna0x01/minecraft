...
public class ListPoolElement extends StructurePoolElement {
	...
	public ListPoolElement(Dynamic<?> dynamic) {
		...
		List<StructurePoolElement> list = dynamic.get("elements")
			.asList(dynamicx -> DynamicDeserializer.deserialize(dynamicx, Registry.field_16793, "element_type", EmptyPoolElement.INSTANCE));
		if (list.isEmpty()) {
			throw new IllegalArgumentException("Elements are empty");
		} else {
			this.elements = list;
	...
	public <T> Dynamic<T> method_16625(DynamicOps<T> dynamicOps) {
		T object = (T)dynamicOps.createList(this.elements.stream().map(structurePoolElement -> structurePoolElement.method_16755(dynamicOps).getValue()));
