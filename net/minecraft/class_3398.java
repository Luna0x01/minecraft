...
public class class_3398 extends DataFix {
	private static final Int2ObjectMap<String> field_16579 = (Int2ObjectMap<String>)DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
	...
	public class_3398(Schema schema, boolean bl) {
		super(schema, bl);
	}
					...
					DSL.remainderFinder(), dynamic -> (Dynamic)dynamic.getMapValues().map(map -> dynamic.createMap((Map)map.entrySet().stream().map(entry -> {
								if (((String)((Dynamic)entry.getKey()).getStringValue().orElse("")).startsWith("key_")) {
									...
									if (i < 0) {
										...
										String string5 = (String)field_16579.getOrDefault(i, "key.unknown");
										return Pair.of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string5));
									}
								} else {
