...
public class DataCommand {
											...
											((RequiredArgumentBuilder)CommandManager.argument("path", NbtPathArgumentType.nbtPath())
													.executes(
														commandContext -> executeGet(
																(ServerCommandSource)commandContext.getSource(), objectType.getObject(commandContext), NbtPathArgumentType.getNbtPath(commandContext, "path")
															)
													))
												.then(
						...
						(argumentBuilder, modifyArgumentCreator) -> argumentBuilder.then(
								...
								.then(
									CommandManager.literal("set")
										.then(modifyArgumentCreator.create((commandContext, compoundTag, nbtPath, list) -> nbtPath.put(compoundTag, ((Tag)Iterables.getLast(list))::copy)))
								)
	...
	private static int executeInsert(int i, CompoundTag compoundTag, NbtPathArgumentType.NbtPath nbtPath, List<Tag> list) throws CommandSyntaxException {
		Collection<Tag> collection = nbtPath.getOrInit(compoundTag, ListTag::new);
		int j = 0;
		...
		for (DataCommand.ObjectType objectType : TARGET_OBJECT_TYPES) {
			objectType.addArgumentsToBuilder(
				...
				argumentBuilder -> {
					...
					biConsumer.accept(
						argumentBuilder2,
						(DataCommand.ModifyArgumentCreator)modifyOperation -> CommandManager.literal("value")
	...
	private static int executeModify(
		...
		if (i == 0) {
			...
			((ServerCommandSource)commandContext.getSource()).sendFeedback(dataCommandObject.feedbackModify(), true);
			return i;
		}
	}
