...
public class TestCommand {
	public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher) {
		commandDispatcher.register(
			(LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal(
													"test"
												)
												.then(CommandManager.literal("runthis").executes(commandContext -> executeRunThis((ServerCommandSource)commandContext.getSource()))))
											.then(CommandManager.literal("runthese").executes(commandContext -> executeRunThese((ServerCommandSource)commandContext.getSource()))))
										.then(
											CommandManager.literal("run")
												.then(
													CommandManager.argument("testName", TestFunctionArgumentType.testFunction())
														.executes(
															commandContext -> executeRun((ServerCommandSource)commandContext.getSource(), TestFunctionArgumentType.getFunction(commandContext, "testName"))
														)
												)
										))
									.then(
										((LiteralArgumentBuilder)CommandManager.literal("runall").executes(commandContext -> executeRunAll((ServerCommandSource)commandContext.getSource())))
											.then(
												CommandManager.argument("testClassName", TestClassArgumentType.method_22370())
													.executes(
														commandContext -> executeRunAll(
																(ServerCommandSource)commandContext.getSource(), TestClassArgumentType.getTestClass(commandContext, "testClassName")
															)
													)
											)
									))
									...
									CommandManager.literal("export")
										.then(
											CommandManager.argument("testName", StringArgumentType.word())
												.executes(
													commandContext -> executeExport((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"))
												)
										)
								))
							.then(
								CommandManager.literal("import")
									.then(
										CommandManager.argument("testName", StringArgumentType.word())
											.executes(commandContext -> executeImport((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName")))
									)
							))
						.then(
							((LiteralArgumentBuilder)CommandManager.literal("pos").executes(commandContext -> executePos((ServerCommandSource)commandContext.getSource(), "pos")))
								.then(
									CommandManager.argument("var", StringArgumentType.word())
										.executes(commandContext -> executePos((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "var")))
								)
						))
					.then(
						CommandManager.literal("create")
							.then(
								((RequiredArgumentBuilder)CommandManager.argument("testName", StringArgumentType.word())
										.executes(
											commandContext -> executeCreate((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"), 5, 5, 5)
										))
									.then(
				...
				.then(
					((LiteralArgumentBuilder)CommandManager.literal("clearall")
							.executes(commandContext -> executeClearAll((ServerCommandSource)commandContext.getSource(), 200)))
						.then(
							CommandManager.argument("radius", IntegerArgumentType.integer())
								.executes(commandContext -> executeClearAll((ServerCommandSource)commandContext.getSource(), IntegerArgumentType.getInteger(commandContext, "radius")))
						)
				)
		);
	}
	...
	private static int executeCreate(ServerCommandSource serverCommandSource, String string, int i, int j, int k) {
		if (i <= 32 && j <= 32 && k <= 32) {
			...
			BlockPos blockPos2 = new BlockPos(
				blockPos.getX(), serverCommandSource.getWorld().getTopPosition(Heightmap.Type.field_13202, blockPos).getY(), blockPos.getZ() + 3
			);
		...
		if (!optional.isPresent()) {
			...
			Text text = new LiteralText(string2)
				.setStyle(
					new Style()
						.setBold(true)
						.setColor(Formatting.field_1060)
						.setHoverEvent(new HoverEvent(HoverEvent.Action.field_11762, new LiteralText("Click to copy to clipboard")))
						.setClickEvent(new ClickEvent(ClickEvent.Action.field_21462, "final BlockPos " + string + " = new BlockPos(" + string2 + ");"))
				);
	...
	private static void onCompletion(ServerWorld serverWorld, TestSet testSet) {
		if (testSet.isDone()) {
			sendMessage(serverWorld, "GameTest done! " + testSet.getTestCount() + " tests were run", Formatting.field_1068);
			if (testSet.failed()) {
				sendMessage(serverWorld, "" + testSet.getFailedRequiredTestCount() + " required tests failed :(", Formatting.field_1061);
			} else {
				sendMessage(serverWorld, "All required tests passed :)", Formatting.field_1060);
			}

			if (testSet.hasFailedOptionalTests()) {
				sendMessage(serverWorld, "" + testSet.getFailedOptionalTestCount() + " optional tests failed", Formatting.field_1080);
			}
		}
	}
	...
	private static int executeClearAll(ServerCommandSource serverCommandSource, int i) {
		...
		BlockPos blockPos = new BlockPos(
			...
			(double)serverCommandSource.getWorld().getTopPosition(Heightmap.Type.field_13202, new BlockPos(serverCommandSource.getPosition())).getY(),
