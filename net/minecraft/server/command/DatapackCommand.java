...
public class DatapackCommand {
									...
									((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument(
														...
														.executes(
															commandContext -> executeEnable(
																	...
																	(list, resourcePackContainer) -> resourcePackContainer.getInitialPosition()
																			.insert(list, resourcePackContainer, resourcePackContainerx -> resourcePackContainerx, false)
																)
														))
													.then(
											...
											.then(
												CommandManager.literal("last")
													.executes(
														commandContext -> executeEnable((ServerCommandSource)commandContext.getSource(), getPackContainer(commandContext, "name", true), List::add)
													)
											))
										.then(
											CommandManager.literal("first")
												.executes(
													commandContext -> executeEnable(
															(ServerCommandSource)commandContext.getSource(),
															getPackContainer(commandContext, "name", true),
															(list, resourcePackContainer) -> list.add(0, resourcePackContainer)
														)
												)
										)
								)
						...
						CommandManager.literal("disable")
							.then(
								CommandManager.argument("name", StringArgumentType.string())
									.suggests(ENABLED_CONTAINERS_SUGGESTION_PROVIDER)
									.executes(commandContext -> executeDisable((ServerCommandSource)commandContext.getSource(), getPackContainer(commandContext, "name", false)))
							)
					))
				.then(
					((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("list")
								.executes(commandContext -> executeList((ServerCommandSource)commandContext.getSource())))
							.then(CommandManager.literal("available").executes(commandContext -> executeListAvailable((ServerCommandSource)commandContext.getSource()))))
						.then(CommandManager.literal("enabled").executes(commandContext -> executeListEnabled((ServerCommandSource)commandContext.getSource())))
				)
		);
	}
	...
	private static int executeEnable(ServerCommandSource serverCommandSource, ResourcePackContainer resourcePackContainer, DatapackCommand.PackAdder packAdder) throws CommandSyntaxException {
		...
		List<ResourcePackContainer> list = Lists.newArrayList(resourcePackContainerManager.getEnabledContainers());
		...
		if (resourcePackContainerManager.getDisabledContainers().isEmpty()) {
			...
			serverCommandSource.sendFeedback(
				new TranslatableText(
					...
					Texts.join(resourcePackContainerManager.getDisabledContainers(), resourcePackContainer -> resourcePackContainer.getInformationText(false))
				),
				false
			);
		}
		...
		if (resourcePackContainerManager.getEnabledContainers().isEmpty()) {
			...
			serverCommandSource.sendFeedback(
				new TranslatableText(
					...
					Texts.join(resourcePackContainerManager.getEnabledContainers(), resourcePackContainer -> resourcePackContainer.getInformationText(true))
				),
				false
			);
		}
	...
	private static ResourcePackContainer getPackContainer(CommandContext<ServerCommandSource> commandContext, String string, boolean bl) throws CommandSyntaxException {
		...
		ResourcePackContainerManager<ResourcePackContainer> resourcePackContainerManager = ((ServerCommandSource)commandContext.getSource())
			.getMinecraftServer()
			.getDataPackContainerManager();
