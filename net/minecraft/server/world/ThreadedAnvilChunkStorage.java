...
public class ThreadedAnvilChunkStorage extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider {
	...
	private ChunkHolder setLevel(long l, int i, @Nullable ChunkHolder chunkHolder, int j) {
		if (j > MAX_LEVEL && i > MAX_LEVEL) {
			...
			if (i <= MAX_LEVEL && chunkHolder == null) {
				chunkHolder = (ChunkHolder)this.field_18807.remove(l);
				if (chunkHolder != null) {
					chunkHolder.setLevel(i);
				} else {
	...
	private boolean save(Chunk chunk) {
		...
		if (!chunk.needsSaving()) {
			...
			try {
				...
				CompoundTag compoundTag2 = ChunkSerializer.serialize(this.world, chunk);
				this.setTagAt(chunkPos, compoundTag2);
				return true;
	...
	public void updateCameraPosition(ServerPlayerEntity serverPlayerEntity) {
		...
		int ix = MathHelper.floor(serverPlayerEntity.getX()) >> 4;
		int j = MathHelper.floor(serverPlayerEntity.getZ()) >> 4;
		ChunkSectionPos chunkSectionPos = serverPlayerEntity.getCameraPosition();
		...
		if (Math.abs(k - ix) <= this.watchDistance * 2 && Math.abs(n - j) <= this.watchDistance * 2) {
			...
			for (int s = o; s <= q; s++) {
				for (int t = p; t <= r; t++) {
					...
					boolean bl5 = getChebyshevDistance(chunkPos, ix, j) <= this.watchDistance;
			...
			for (int w = ix - this.watchDistance; w <= ix + this.watchDistance; w++) {
				for (int x = j - this.watchDistance; x <= j + this.watchDistance; x++) {
					...
					boolean bl8 = false;
					boolean bl9 = true;
					this.sendWatchPackets(serverPlayerEntity, chunkPos3, new Packet[2], false, true);
				}
			}
		}
	}
	...
	protected void tickPlayerMovement() {
		...
		if (!list.isEmpty()) {
			...
			while (var3.hasNext()) {
				ThreadedAnvilChunkStorage.EntityTracker entityTracker2 = (ThreadedAnvilChunkStorage.EntityTracker)var3.next();
				entityTracker2.updateCameraPosition(list);
			}
		}
	}
	...
	private void sendChunkDataPackets(ServerPlayerEntity serverPlayerEntity, Packet<?>[] packets, WorldChunk worldChunk) {
		...
		if (!list2.isEmpty()) {
			for (Entity entity3 : list2) {
				serverPlayerEntity.networkHandler.sendPacket(new EntityPassengersSetS2CPacket(entity3));
			}
		}
	}
