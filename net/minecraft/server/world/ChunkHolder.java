...
public class ChunkHolder {
	...
	public void flushUpdates(WorldChunk worldChunk) {
		if (this.blockUpdateCount != 0 || this.skyLightUpdateBits != 0 || this.blockLightUpdateBits != 0) {
			...
			if (this.blockUpdateCount == 1) {
				int k = (this.blockUpdatePositions[0] >> 12 & 15) + this.pos.x * 16;
				int l = this.blockUpdatePositions[0] & 255;
				...
				for (int n = 0; n < this.blockUpdateCount; n++) {
					int o = (this.blockUpdatePositions[n] >> 12 & 15) + this.pos.x * 16;
	...
	protected void tick(ThreadedAnvilChunkStorage threadedAnvilChunkStorage) {
		...
		if (bl3 && !bl4) {
			CompletableFuture<Either<WorldChunk, ChunkHolder.Unloaded>> completableFuture2 = this.borderFuture;
			this.borderFuture = UNLOADED_WORLD_CHUNK_FUTURE;
			this.updateFuture(completableFuture2.thenApply(either -> either.ifLeft(threadedAnvilChunkStorage::method_20576)));
		}
