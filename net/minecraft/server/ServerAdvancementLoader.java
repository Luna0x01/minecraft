...
public class ServerAdvancementLoader extends JsonDataLoader {
	...
	private static final Gson GSON = new GsonBuilder()
		.registerTypeHierarchyAdapter(Advancement.Task.class, (JsonDeserializer)(jsonElement, type, jsonDeserializationContext) -> {
	...
	protected void method_20724(Map<Identifier, JsonObject> map, ResourceManager resourceManager, Profiler profiler) {
		...
		map.forEach((identifier, jsonObject) -> {
			try {
				Advancement.Task task = (Advancement.Task)GSON.fromJson(jsonObject, Advancement.Task.class);
