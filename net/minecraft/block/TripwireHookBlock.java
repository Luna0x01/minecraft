...
public class TripwireHookBlock extends Block {
	...
	public void update(World world, BlockPos pos, BlockState state, boolean beingRemoved, boolean bl, int i, @Nullable BlockState blockState) {
		...
		if (j > 0) {
			BlockPos blockPos2 = pos.offset(direction, j);
			Direction direction2 = direction.getOpposite();
			world.setBlockState(blockPos2, blockState3.with(FACING, direction2), 3);
		...
		if (bl2 != bl4) {
			for (int l = 1; l < j; l++) {
				...
				if (blockState4 != null) {
					world.setBlockState(blockPos3, blockState4.with(ATTACHED, Boolean.valueOf(bl4)), 3);
