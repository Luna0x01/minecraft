...
public class NetherPortalBlock extends Block {
	...
	public void scheduledTick(BlockState blockState, ServerWorld serverWorld, BlockPos blockPos, Random random) {
		if (serverWorld.dimension.hasVisibleSky()
			&& serverWorld.getGameRules().getBoolean(GameRules.field_19390)
			&& random.nextInt(2000) < serverWorld.getDifficulty().getId()) {
			while (serverWorld.getBlockState(blockPos).getBlock() == this) {
				blockPos = blockPos.down();
			}

			if (serverWorld.getBlockState(blockPos).allowsSpawning(serverWorld, blockPos, EntityType.field_6050)) {
				Entity entity = EntityType.field_6050.spawn(serverWorld, null, null, null, blockPos.up(), SpawnType.field_16474, false, false);
				if (entity != null) {
	...
	public boolean createPortalAt(IWorld iWorld, BlockPos blockPos) {
		NetherPortalBlock.AreaHelper areaHelper = this.createAreaHelper(iWorld, blockPos);
		if (areaHelper != null) {
	...
	@Nullable
	public NetherPortalBlock.AreaHelper createAreaHelper(IWorld iWorld, BlockPos blockPos) {
		NetherPortalBlock.AreaHelper areaHelper = new NetherPortalBlock.AreaHelper(iWorld, blockPos, Direction.Axis.field_11048);
		if (areaHelper.isValid() && areaHelper.foundPortalBlocks == 0) {
			return areaHelper;
		} else {
			NetherPortalBlock.AreaHelper areaHelper2 = new NetherPortalBlock.AreaHelper(iWorld, blockPos, Direction.Axis.field_11051);
	...
	public BlockState getStateForNeighborUpdate(
		...
		return !bl && blockState2.getBlock() != this && !new NetherPortalBlock.AreaHelper(iWorld, blockPos, axis2).wasAlreadyValid()
	...
	public static BlockPattern.Result findPortal(IWorld iWorld, BlockPos blockPos) {
		...
		if (!areaHelper.isValid()) {
			...
			areaHelper = new NetherPortalBlock.AreaHelper(iWorld, blockPos, Direction.Axis.field_11051);
		}
	...
	public static class AreaHelper {
		...
		protected int findHeight() {
			...
			for (int j = 0; j < this.width; j++) {
				if (this.world.getBlockState(this.lowerCorner.offset(this.negativeDir, j).up(this.height)).getBlock() != Blocks.field_10540) {
					this.height = 0;
					break;
				}
			}
		...
		public void createPortal() {
			for (int i = 0; i < this.width; i++) {
				...
				for (int j = 0; j < this.height; j++) {
					this.world.setBlockState(blockPos.up(j), Blocks.field_10316.getDefaultState().with(NetherPortalBlock.AXIS, this.axis), 18);
				}
			}
		}
