...
public class ItemLoreToComponentFix extends DataFix {
					...
					typedx -> typedx.update(
							...
							dynamic -> dynamic.update(
									...
									dynamicx -> dynamicx.update(
											"Lore",
											dynamicxx -> (Dynamic)DataFixUtils.orElse(dynamicxx.asStreamOpt().map(ItemLoreToComponentFix::fixLoreTags).map(dynamicxx::createList), dynamicxx)
										)
								)
						)
				)
	...
	private static <T> Stream<Dynamic<T>> fixLoreTags(Stream<Dynamic<T>> stream) {
		return stream.map(dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asString().map(ItemLoreToComponentFix::componentize).map(dynamic::createString), dynamic));
	}
